...
struct Company: Codable, Identifiable, Hashable {
    ...
    /// Quick init for minimal company references
    static func stub(company: String?, ticker: String?, composite: Double?, hi_balanced: Bool? = nil) -> Company {
        Company(company: company, ticker: ticker, composite: composite, industry: nil, sic_description: nil, hi_balanced: hi_balanced, hi_grade: nil, D_H: nil, D_U: nil, D_M: nil, D_A: nil, D_N: nil, decay_index: nil, decay_level: nil, shield_score: nil, shield_tier: nil, genome: nil, algo_harm: nil, humanwashing_flags: nil, domains: nil, data_sources: nil, confidence: nil, _source: nil)
    }
}
...
struct ContagionEntry: Codable, Identifiable {
    var id: String { ticker ?? company ?? UUID().uuidString }
    let company: String?; let ticker: String?; let contagion_score: Double?
    let contagion_type: String?; let industry: String?; let composite: Double?
}
