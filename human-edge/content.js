    ...
    return `
      <div class="human-badge__mini" style="padding:0;background:transparent !important;border:none !important;box-shadow:none !important">
        <svg width="56" height="70" viewBox="0 -4 68 88" style="animation:gold-glow 3s ease-in-out infinite;filter:drop-shadow(0 2px 8px rgba(196,155,32,0.4))">
          ...
          <text x="34" y="61" text-anchor="middle" fill="#C49B20" font-family="-apple-system,BlinkMacSystemFont,sans-serif" font-size="10" font-weight="900">hi.</text>
        </svg>
      </div>
    `;
  }
...
function openFullPanel(profile, filterResult, prefs) {
  ...
  panel.innerHTML = `
    ...
    <div class="human-panel__company" style="background:white">
      <div class="human-panel__grade" style="${profile.hiBalanced ? 'background:#C49B20;color:white;border-radius:50%;width:64px;height:64px;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 0 16px rgba(196,155,32,0.3)' : 'color:'+scoreColor+';font-size:36px'}">${profile.hiBalanced ? '<img src="'+chrome.runtime.getURL('icons/icon-white-128.png')+'" style="height:32px;width:auto">' : profile.composite}</div>
      <div>
    ...
    ${(() => {
      ...
      return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span style="font-size:11px;font-weight:700;color:#1B3A5C">' + passed + '/' + total + ' GATES</span><div style="flex:1;height:4px;background:#EEF1F5;border-radius:2px"><div style="height:100%;width:' + (passed/total*100) + '%;background:' + (passed === total ? '#C49B20' : '#1B3A5C') + ';border-radius:2px"></div></div></div>' +
